...
export async function runStartupMigrations(): Promise<void> {
  ...
  try {
    ...
    // ── FIX-AP: appointments.client_id → nullable + guest contact columns ─────
    // Appointments can now be booked for guests without a CRM client record
    // (client_id = NULL, contact captured in guest_name/guest_phone/guest_email).
    {
      await db.execute(sql`
