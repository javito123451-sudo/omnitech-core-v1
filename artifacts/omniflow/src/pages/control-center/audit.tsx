...
import {
  ClipboardList, AlertTriangle, Info, ShieldAlert, Clock, Loader2, Filter,
  Search, Download, ChevronLeft, ChevronRight, X, Calendar, Building2,
} from "lucide-react";
...
interface AuditResponse { logs: AuditLog[]; total: number; limit: number; offset: number; }
...
const ACTION_LABELS: Record<string, string> = {
  ...
  whatsapp_message_received:   "Mensaje WhatsApp recibido",
  whatsapp_quote_accepted:     "Presupuesto aceptado (WhatsApp)",
  whatsapp_quote_rejected:     "Presupuesto rechazado (WhatsApp)",
};
...
export default function AuditPage() {
  ...
  const buildParams = useCallback(() => {
    ...
    return p.toString();
  }, [severity, action, actor, orgId, startDate, endDate, page]);

  const { data, isLoading, isFetching } = useQuery<AuditResponse>({
    queryKey: ["cc-audit", severity, action, actor, orgId, startDate, endDate, page],
  ...
  const hasFilters = severity !== "all" || action || actor || orgId || startDate || endDate;

  const resetFilters = () => {
    setSeverity("all"); setAction(""); setActor(""); setOrgId("");
  ...
  const infoCount  = total > 0 ? undefined : logs.filter(l => l.severity === "info").length;
  const warnCount  = total > 0 ? undefined : logs.filter(l => l.severity === "warning").length;
  const critCount  = total > 0 ? undefined : logs.filter(l => l.severity === "critical").length;
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-3">
            <ClipboardList size={24} className="text-violet-400" /> Auditoría
          </h1>
      ...
      {/* Severity quick-filter cards — counts from total, not page */}
      <div className="grid grid-cols-3 gap-4 mb-6">
        {(["info", "warning", "critical"] as const).map(sev => {
          const style = SEVERITY_STYLES[sev];
          const Icon  = style.icon;
          const label = sev === "info" ? "Informativos" : sev === "warning" ? "Advertencias" : "Críticos";
          return (
            <button
              key={sev}
            ...
            >
              ...
              <p className={`text-2xl font-bold ${style.text}`}>
                {severity === sev ? total.toLocaleString() : "—"}
              </p>
              <p className="text-slate-500 text-xs mt-1">{label}</p>
              <p className="text-slate-600 text-xs mt-0.5">
                {severity === sev ? "filtro activo" : "clic para filtrar"}
              </p>
            </button>
          );
        })}
      ...
      <div className="bg-[#0d0e1e] border border-white/[0.06] rounded-2xl p-4 mb-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3">
          ...
          <div className="relative">
            ...
            <input
              type="number" value={orgId} onChange={e => { setOrgId(e.target.value); setPage(0); }}
              placeholder="Org ID..."
              className="w-full bg-white/5 border border-white/[0.06] rounded-xl pl-9 pr-3 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:border-violet-500 text-sm"
            />
          </div>
        ...
        {hasFilters && (
          <div className="flex items-center gap-2 mt-3 pt-3 border-t border-white/[0.04] flex-wrap">
            <span className="text-xs text-slate-500 flex items-center gap-1.5"><Filter size={12} /> Filtros:</span>
            ...
            {orgId     && <Chip label={`Org: ${orgId}`}      onRemove={() => setOrgId("")}     />}
                ...
                return (
                  <div key={log.id}>
                    <button
                      ...
                      className="w-full px-6 py-4 flex items-start gap-4 hover:bg-white/[0.01] transition-colors text-left"
                    >
                      ...
                      <div className="flex items-center gap-3 flex-shrink-0 text-right">
                        {log.ipAddress && <span className="text-slate-700 text-xs font-mono hidden xl:block">{log.ipAddress}</span>}
                    ...
                    {isOpen && log.details && Object.keys(log.details).length > 0 && (
                      <div className="px-6 pb-4 ml-12">
                        <pre className="text-xs text-slate-400 bg-white/[0.03] border border-white/[0.06] rounded-xl p-4 overflow-x-auto">
                          {JSON.stringify(log.details, null, 2)}
                        </pre>
                        {log.ipAddress && (
                          <p className="text-xs text-slate-600 mt-2">IP: <span className="font-mono text-slate-500">{log.ipAddress}</span></p>
                        )}
                      </div>
                    )}
                  </div>
