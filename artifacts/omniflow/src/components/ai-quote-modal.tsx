...
import {
  X, Sparkles, RefreshCw, Save, Mail, MessageCircle,
  FileDown, CheckCircle2, ChevronRight, Building2, User,
} from "lucide-react";
...
interface AIQuoteResult {
  ...
  items: QuoteItem[];
  notes: string;
  client: { id: number; name: string; company?: string | null; email: string; phone?: string | null };
  validUntil: string;
  generatedAt: string;
}
interface SavedQuote { id: number; title: string; total: number }
...
function QuotePreview({ q, quoteNum }: { q: AIQuoteResult; quoteNum: string }) {
  ...
  return (
    <div className="bg-white text-slate-900 rounded-xl overflow-hidden text-[12px] shadow-lg" id="quote-preview">
      {/* Header */}
      <div className="bg-gradient-to-r from-slate-800 to-slate-900 text-white px-6 py-5 flex items-start justify-between gap-4">
        <div>
          ...
          <div className="text-xs text-slate-400 mt-0.5">Plataforma de Automatización Empresarial</div>
        </div>
        <div className="text-right">
          <div className="text-[10px] text-slate-400 uppercase tracking-wider">Presupuesto</div>
      ...
      <div className="grid grid-cols-2 gap-0 border-b border-slate-200">
        <div className="px-6 py-4 border-r border-slate-200">
          <div className="text-[9px] uppercase tracking-widest text-slate-400 mb-2 font-semibold">Para</div>
          <div className="font-bold text-slate-800 text-sm">{q.client.name}</div>
          {q.client.company && <div className="text-slate-500">{q.client.company}</div>}
          <div className="text-slate-500 mt-1">{q.client.email}</div>
          {q.client.phone && <div className="text-slate-500">{q.client.phone}</div>}
        </div>
        <div className="px-6 py-4">
          <div className="text-[9px] uppercase tracking-widest text-slate-400 mb-2 font-semibold">Detalles</div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b-2 border-slate-200">
              <th className="text-left py-2 text-[9px] uppercase tracking-widest text-slate-400 font-semibold">Descripción</th>
              <th className="text-center py-2 text-[9px] uppercase tracking-widest text-slate-400 font-semibold w-14">Cant.</th>
              <th className="text-right py-2 text-[9px] uppercase tracking-widest text-slate-400 font-semibold w-24">Precio unit.</th>
              <th className="text-right py-2 text-[9px] uppercase tracking-widest text-slate-400 font-semibold w-24">Total</th>
            </tr>
          </thead>
          <tbody>
            {q.items.map((item, i) => (
              <tr key={i} className="border-b border-slate-100">
                <td className="py-2.5 text-slate-700 leading-snug pr-4">{item.description}</td>
                <td className="py-2.5 text-center text-slate-600">{item.quantity}</td>
                <td className="py-2.5 text-right text-slate-600">{fmtEur(item.unitPrice)}</td>
                <td className="py-2.5 text-right font-semibold text-slate-800">{fmtEur(item.quantity * item.unitPrice)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {q.notes && (
        <div className="px-6 pb-5 border-t border-slate-100 pt-4">
          <div className="text-[9px] uppercase tracking-widest text-slate-400 mb-2 font-semibold">Condiciones y alcance</div>
          <div className="text-slate-600 leading-relaxed whitespace-pre-wrap">{q.notes}</div>
        </div>
      )}

      {/* Footer */}
      <div className="bg-slate-50 border-t border-slate-200 px-6 py-3 flex justify-between items-center">
        <div className="text-[10px] text-slate-400">Generado con OmniTech Core</div>
        <div className="text-[10px] text-slate-400">Válido 30 días desde la fecha de emisión</div>
      </div>
    </div>
  );
}
...
export function AIQuoteModal({
  ...
  const generate = useCallback(async () => {
    if (!service.trim()) { setError("Describe el servicio a presupuestar"); return; }
    setError(null);
    setStep("generating");
    try {
  ...
  const sendEmail = () => {
    ...
    const subtotal = result.items.reduce((s, i) => s + i.quantity * i.unitPrice, 0);
    const total    = subtotal * 1.21;
    const body     = encodeURIComponent(
      "Estimado/a " + clientName + ",\n\n" +
      "Adjunto encontrará el presupuesto #" + String(saved.id).padStart(5, "0") + " por importe de " +
      fmtEur(total) + ".\n\n" +
      "Título: " + result.title + "\n" +
      "Válido hasta: " + fmtDate(result.validUntil) + "\n\n" +
  ...
  const shareWhatsApp = () => {
    ...
    const text = encodeURIComponent(
      "Hola " + clientName + ", te envío el presupuesto #" + String(saved.id).padStart(5, "0") +
      " por " + fmtEur(total) + ". " +
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.07] bg-gradient-to-r from-primary/10 to-slate-950">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[10px] font-black uppercase tracking-widest text-primary/70">Presupuesto IA</div>
        ...
        <div className="flex items-center gap-0 px-6 py-3 border-b border-white/[0.05] bg-white/[0.01]">
          {(["input", "preview", "saved"] as const).map((s, i) => {
            ...
            const done   = step === "preview" ? i <= 1 : step === "saved" || step === "saving" ? true : i === 0;
            const active = step === "generating" ? i === 0 : step === "saving" ? i === 1 : step === s;
            return (
              <div key={s} className="flex items-center gap-0">
                {i > 0 && <ChevronRight className="w-3 h-3 text-white/10 mx-1" />}
                <div className={cn("flex items-center gap-1.5 text-[11px] font-semibold transition-colors",
                  done   ? "text-primary"           :
              ...
              <motion.div key="input" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3 p-3.5 rounded-xl border border-white/[0.07] bg-white/[0.02]">
                    ...
                    {clientCompany && (
                      <div className="ml-auto flex items-center gap-1 text-[11px] text-muted-foreground">
                        <Building2 className="w-3 h-3" />{clientCompany}
                      </div>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    />
                    <p className="text-[10px] text-muted-foreground/60 mt-1.5">La IA ajustará los precios de cada partida para aproximarse a este total.</p>
                  </div>
                  ...
                  >
                    {step === "generating" ? (
                      <><RefreshCw className="w-4 h-4 animate-spin" />Generando presupuesto…</>
                    ) : (
                  ...
                  {step === "saved" && saved ? (
                    <div className="space-y-3">
                      <div className="flex items-center gap-2 text-sm text-emerald-400 font-semibold">
                        ...
                        Guardado como #{String(saved.id).padStart(5, "0")} — {fmtEur(saved.total)}
                      </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        {step === "saving" ? (
                          <><RefreshCw className="w-3.5 h-3.5 animate-spin" />Guardando en CRM…</>
                        ) : (
                          <><Save className="w-3.5 h-3.5" />💾 Guardar en CRM</>
                        )}
                      </button>
                    </div>
                  )}
