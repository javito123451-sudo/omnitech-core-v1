...
export async function runStartupMigrations(): Promise<void> {
  ...
  try {
    ...
    // ── FIX-Z: Production data reset — wipe all test/development data ─────────
    // Idempotency guard: only runs if test orgs (13,17,22,23) still exist
    // OR org 7 still has clients from development OR audit_logs are present.
    // After a successful run all three conditions are false → never runs again.
    // PROTECTS: org 7, users 6 (Javier/SUPER_ADMIN) and 14 (Felipe/SUPER_ADMIN),
    //           org_integrations of org 7 (Telegram + WhatsApp credentials intact).
    {
      const check = await db.execute(sql`
        SELECT 1 FROM (
          SELECT 1 WHERE EXISTS (SELECT 1 FROM organizations WHERE id IN (13,17,22,23))
          UNION ALL
          SELECT 1 WHERE EXISTS (SELECT 1 FROM clients   WHERE org_id = 7)
          UNION ALL
          SELECT 1 WHERE EXISTS (SELECT 1 FROM audit_logs LIMIT 1)
        ) _guard LIMIT 1
      `);
      const needsReset = ((check as { rows: unknown[] }).rows).length > 0;

      if (!needsReset) {
        logger.info("[Migration] ✅ FIX-Z: Platform already clean — skipping reset");
      } else {
        logger.info("[Migration] 🧹 FIX-Z: Starting production data reset…");

        // ── Step 1: Clean org 7 business data (FK order: children first) ───────

        // Children of ai_sessions
        await db.execute(sql`DELETE FROM ai_messages WHERE session_id IN (SELECT id FROM ai_sessions WHERE org_id = 7)`);
        // Children of quotes
        await db.execute(sql`DELETE FROM quote_items WHERE quote_id IN (SELECT id FROM quotes WHERE org_id = 7)`);
        // Children of invoices
        await db.execute(sql`DELETE FROM invoice_items WHERE invoice_id IN (SELECT id FROM invoices WHERE org_id = 7)`);
        await db.execute(sql`DELETE FROM accounting_payments WHERE org_id = 7`);
        await db.execute(sql`DELETE FROM credit_notes WHERE org_id = 7`);
        // Children of lead_searches
        await db.execute(sql`DELETE FROM lead_results WHERE search_id IN (SELECT id FROM lead_searches WHERE org_id = 7)`);
        // Children of agent_memory
        await db.execute(sql`DELETE FROM memory_history WHERE org_id = 7`);
        // Children of deals
        await db.execute(sql`DELETE FROM deals WHERE org_id = 7`);
        // Children of ads_campaigns
        await db.execute(sql`DELETE FROM ads_creatives WHERE campaign_id IN (SELECT id FROM ads_campaigns WHERE org_id = 7)`);
        // Children of support_tickets
        await db.execute(sql`DELETE FROM ticket_comments WHERE ticket_id IN (SELECT id FROM support_tickets WHERE org_id = 7)`);

        // Level 2: standalone business tables
        await db.execute(sql`DELETE FROM ai_sessions       WHERE org_id = 7`);
        ...
        await db.execute(sql`DELETE FROM invoices          WHERE org_id = 7`);
        await db.execute(sql`DELETE FROM recurring_invoices WHERE org_id = 7`);
        await db.execute(sql`DELETE FROM expenses          WHERE org_id = 7`);
        await db.execute(sql`DELETE FROM lead_searches     WHERE org_id = 7`);
        ...
        await db.execute(sql`DELETE FROM pipeline_stages   WHERE org_id = 7`);
        await db.execute(sql`DELETE FROM integration_events WHERE org_id = 7`);
        await db.execute(sql`DELETE FROM marketing_campaigns WHERE org_id = 7`);
        await db.execute(sql`DELETE FROM ads_campaigns     WHERE org_id = 7`);
        await db.execute(sql`DELETE FROM support_tickets   WHERE org_id = 7`);
        await db.execute(sql`DELETE FROM tax_obligations   WHERE org_id = 7`);
        await db.execute(sql`DELETE FROM tax_health_score  WHERE org_id = 7`);

        // Level 3: clients
        await db.execute(sql`DELETE FROM clients WHERE org_id = 7`);

        // Level 4: org 7 memberships — keep SUPER_ADMINs user_id 6 and 14
        await db.execute(sql`DELETE FROM org_members WHERE org_id = 7 AND user_id NOT IN (6, 14)`);

        logger.info("[Migration] ✅ FIX-Z: Org 7 business data cleaned (integrations intact)");

        // ── Step 2: Eliminate test workspaces (13, 17, 22, 23) ──────────────────
        for (const testOrgId of [13, 17, 22, 23]) {
          const exists = await db.execute(sql`SELECT 1 FROM organizations WHERE id = ${testOrgId} LIMIT 1`);
          if (((exists as { rows: unknown[] }).rows).length === 0) continue;

          await db.execute(sql`DELETE FROM ai_messages   WHERE session_id IN (SELECT id FROM ai_sessions WHERE org_id = ${testOrgId})`);
          await db.execute(sql`DELETE FROM ai_sessions   WHERE org_id = ${testOrgId}`);
          await db.execute(sql`DELETE FROM ai_usage_logs WHERE org_id = ${testOrgId}`);
          await db.execute(sql`DELETE FROM agent_memory  WHERE org_id = ${testOrgId}`);
          await db.execute(sql`DELETE FROM quote_items   WHERE quote_id IN (SELECT id FROM quotes WHERE org_id = ${testOrgId})`);
          await db.execute(sql`DELETE FROM invoice_items WHERE invoice_id IN (SELECT id FROM invoices WHERE org_id = ${testOrgId})`);
          await db.execute(sql`DELETE FROM accounting_payments WHERE org_id = ${testOrgId}`);
          await db.execute(sql`DELETE FROM quotes        WHERE org_id = ${testOrgId}`);
          await db.execute(sql`DELETE FROM invoices      WHERE org_id = ${testOrgId}`);
          await db.execute(sql`DELETE FROM clients       WHERE org_id = ${testOrgId}`);
          await db.execute(sql`DELETE FROM appointments  WHERE org_id = ${testOrgId}`);
          await db.execute(sql`DELETE FROM messages      WHERE org_id = ${testOrgId}`);
          await db.execute(sql`DELETE FROM activity      WHERE org_id = ${testOrgId}`);
          await db.execute(sql`DELETE FROM integration_events WHERE org_id = ${testOrgId}`);
          await db.execute(sql`DELETE FROM org_integrations   WHERE org_id = ${testOrgId}`);
          await db.execute(sql`DELETE FROM org_members         WHERE org_id = ${testOrgId}`);
          await db.execute(sql`DELETE FROM organizations        WHERE id    = ${testOrgId}`);

          logger.info(`[Migration] ✅ FIX-Z: Test workspace org_id=${testOrgId} eliminated`);
        }

        // ── Step 3: Remove test users (keep SUPER_ADMINs id=6 and id=14) ────────
        // All org_members for test users have been deleted above — safe to remove users.
        const deleted = await db.execute(sql`DELETE FROM users WHERE id NOT IN (6, 14)`);
        const delCount = (deleted as { rowCount?: number }).rowCount ?? 0;
        logger.info(`[Migration] ✅ FIX-Z: ${delCount} test user(s) removed — kept IDs 6 and 14`);

        // ── Step 4: Clear audit_logs (fresh start for production) ────────────────
        await db.execute(sql`DELETE FROM audit_logs`);
        logger.info("[Migration] ✅ FIX-Z: Audit logs cleared — production starts clean");

        // ── Step 5: Clear onboard wizard drafts ─────────────────────────────────
        await db.execute(sql`DELETE FROM onboard_wizard_drafts`);
        logger.info("[Migration] ✅ FIX-Z: Wizard drafts cleared");

        logger.info("[Migration] ✅ FIX-Z: Production data reset complete — platform ready for real clients");
      }
    }
