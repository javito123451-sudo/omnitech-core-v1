...
knowledgeBaseRouter.get("/", requirePermission("ai.read"), async (req, res) => {
...
knowledgeBaseRouter.post("/", requirePermission("ai.write"), async (req, res) => {
  const orgId = (req as any).orgId as number;
...
knowledgeBaseRouter.put("/:id", requirePermission("ai.write"), async (req, res) => {
  const orgId = (req as any).orgId as number;
...
knowledgeBaseRouter.delete("/:id", requirePermission("ai.write"), async (req, res) => {
  const orgId = (req as any).orgId as number;
  const id = Number(req.params.id);
  try {
...
knowledgeBaseRouter.get("/categories", requirePermission("ai.read"), async (req, res) => {
  const orgId = (req as any).orgId as number;
  try {
    const entries = await db
